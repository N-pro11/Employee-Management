if (!localStorage.getItem("lastAssignedId")) {
  localStorage.setItem("lastAssignedId", "30"); // Start from 0 or any other initial value
}

$(document).ready(function () {
  $(document).on("click", ".edit-btn", function () {
    const id = $(this).data("id"); // Get the ID from the data-id attribute
    editRow(id); // Call the editRow function
...
//form validation and add new user
    ...
    content += `
      <tr id="row${user.id}">
          ...
          <td>
              ...
              <button class="btn btn-outline-danger del-btn" data-id="${
                user.id
              }">Delete</button>
          </td>
      </tr>`;
